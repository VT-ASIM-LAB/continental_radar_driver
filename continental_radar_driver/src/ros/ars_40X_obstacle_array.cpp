...
namespace ars_40X
{
    ObstacleArray::ObstacleArray() {
        ...
        object_list_sub_ = nh.subscribe("objects", 10, &ObstacleArray::object_list_callback, this);

        obstacle_array_pub_ = nh.advertise<costmap_converter::ObstacleArrayMsg>("obstacles", 10);
    }
    ...
    void ObstacleArray::object_list_callback(ars_40X::ObjectList object_list) {
        ...
        for (auto object : object_list.objects) {
            ...
            tf2::Quaternion q(object.position.pose.orientation.x,
            ...
            obstacle.orientation = object.position.pose.orientation;
